import { DeepPartial, FindConditions, FindManyOptions, getMongoRepository, MongoRepository } from 'typeorm';
...
export abstract class GenericSM<TDo, TId, TRepository extends MongoRepository<TDo>> {
  ...
  async partialUpdate(_id: ObjectID, partialEntity: DeepPartial<TDo>): Promise<any> {
    try {
      ...
      const updated = await this.repository.findOneAndUpdate({ _id }, { $set: partialEntity }, option)
  ...
  findAll(options, name): Promise<any> {
    const { take = 100, skip = 1, order, where = {}, search, relation, match, aggregate = [{ $match: {} }] } = options;  
    const aggregate_search = search ? { $text: { $search: search } } : {}
    return this.repository.aggregate([
      { $match: aggregate_search },
      ...aggregate,
